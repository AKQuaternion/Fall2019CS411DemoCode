// build_test_timed.cpp
// VERSION 5
// Based on build_test.cpp VERSION 2
// Glenn G. Chappell
// 11 Oct 2015
//
...
// Used in GRADING Assignment 2, Exercise A
...
public:
  ...
  void test(bool success, const std::string &testName = "") {
    ++countTests_;
    if (success)
      ++countPasses_;
  ...
  //      and pass/fail printed to cout
  // Does not throw (No-Throw Guarantee)
  void ftest(double val1, double val2, const std::string &testName = "") {
  ...
  //     return == countTests_
  // Does not throw (No-Throw Guarantee)
  int numTests() const { return countTests_; }
  ...
  //     return == countPasses_
  // Does not throw (No-Throw Guarantee)
  int numPassed() const { return countPasses_; }
  ...
  // Does not throw (No-Throw Guarantee)
  int numFailed() const { return countTests_ - countPasses_; }
  ...
  // Does not throw (No-Throw Guarantee)
  bool allPassed() const { return countPasses_ == countTests_; }
...
public:
  ...
  //     Return is true.
  // Does not throw (No-Throw Guarantee)
  static bool check(const T &param) { return true; }
  ...
  // Requirements on types: None.
  // Does not throw (No-Throw Guarantee)
  template <typename OtherType> static bool check(const OtherType &param) {
    return false;
  }
...
public:
  ...
  //     return == existing_.
  // Does not throw (No-Throw Guarantee)
  static int getExisting() { return existing_; }
  ...
  //     return == maxExisting_.
  // Does not throw (No-Throw Guarantee)
  static int getMaxExisting() { return maxExisting_; }
  ...
  //     return == ctorCount_.
  // Does not throw (No-Throw Guarantee)
  static int getCtorCount() { return ctorCount_; }
  ...
  //     return == dctorCount_.
  // Does not throw (No-Throw Guarantee)
  static int getDctorCount() { return dctorCount_; }
  ...
  //     return == assnCount_.
  // Does not throw (No-Throw Guarantee)
  static int getAssnCount() { return assnCount_; }
  ...
  //     copyThrow_ == shouldThrow
  // Does not throw (No-Throw Guarantee)
  static void setCopyThrow(bool shouldThrow) { copyThrow_ = shouldThrow; }
...
//     Return value == false.
// Does not throw (No-Throw Guarantee)
bool operator<(const Counter &a, const Counter &b) { return false; }
...
void test_build_single(Tester &t,
  ...
  if (!success) {
    std::cout << "      Expected return value: " << ans << std::endl;
    std::cout << "      Actual return value: " << retval << std::endl;
  }
}
...
void test_build_tiny(Tester &t) {
  std::cout << "Test Suite: function build - tiny examples" << std::endl;
  ...
  // Empty list of bridges
  {
    ...
    bs.clear();
    ans = 0;
    test_build_single(t, w, e, cbs, ans, "No bridges");
  }
  ...
  // 1 bridge
  {
    ...
    ans = 13;
    test_build_single(t, w, e, cbs, ans, "1 bridge");
  }
  ...
  // 2 bridges, non-crossing
  {
    ...
    ans = 11;
    test_build_single(t, w, e, cbs, ans, "2 bridges, non-crossing");
  }
  ...
  // 2 bridges, identical
  {
    ...
    bs.push_back(Brg{0, 2, 5});
    ans = 5;
    test_build_single(t, w, e, cbs, ans, "2 bridges, identical");
  }
  ...
  // 2 bridges, crossing, NW-SE is best
  {
    ...
    ans = 6;
    test_build_single(t, w, e, cbs, ans, "2 bridges, crossing, NW-SE is best");
  }
  ...
  // 2 bridges, crossing, SW-NE is best
  {
    ...
    ans = 7;
    test_build_single(t, w, e, cbs, ans, "2 bridges, crossing, SW-NE is best");
  }
...
void test_build_small(Tester &t) {
  std::cout << "Test Suite: function build - small examples" << std::endl;
  ...
  // Greedy fails large
  {
    ...
    ans = w;
    test_build_single(t, w, e, cbs, ans, "Greedy fails large");
  }
  ...
  // 3 cities each side, all joinable
  {
    ...
    ans = 12;
    test_build_single(t, w, e, cbs, ans, "3 cities each side, all joinable");
  }
}
...
void test_build_medium(Tester &t) {
  std::cout << "Test Suite: function build - medium examples" << std::endl;
  ...
  // All bridges available #1
  {
    ...
    ans = w * 3;
    test_build_single(t, w, e, cbs, ans, "All bridges available #1");
  }
  ...
  // All bridges available #2
  {
    ...
    ans = (w - 1) * 5;
    test_build_single(t, w, e, cbs, ans, "All bridges available #2");
  }
  ...
  // All bridges available #3
  {
    ...
    ans = (w - 2) * 7;
    test_build_single(t, w, e, cbs, ans, "All bridges available #3");
  }
  ...
  // Random #1
  {
    ...
    ans = 31;
    test_build_single(t, w, e, cbs, ans, "Random #1");
  }
  ...
  // Random #2
  {
    ...
    ans = 16;
    test_build_single(t, w, e, cbs, ans, "Random #2");
  }
  ...
  // Random #3
  {
    ...
    ans = 26;
    test_build_single(t, w, e, cbs, ans, "Random #3");
  }
  ...
  // One bridge crosses many #1
  {
    ...
    ans = w + 2;
    test_build_single(t, w, e, cbs, ans, "One bridge crosses many #1");
  }
  ...
  // One bridge crosses many #2
  {
    ...
    ans = w;
    test_build_single(t, w, e, cbs, ans, "One bridge crosses many #2");
  }
}
...
// test_build_EXTRA
// Test suite for function build: EXTRA examples
// Pre: None.
// Post:
//     Pass/fail status of tests have been registered with t.
//     Appropriate messages have been printed to cout.
// Does not throw (No-Throw Guarantee)
void test_build_EXTRA(Tester &t) {
  std::cout << "Test Suite: function build - EXTRA examples" << std::endl;
  ...
  // Crossing many
  {
    ...
    bs.push_back(Brg{a - 1, 0, a + 1});
    ans = a + 1;
    test_build_single(t, w, e, cbs, ans, "Crossing many");
  }
  ...
  // All bridges available #3 - multiple
  {
    for (int count = 0; count < 100; ++count) {
      ...
      ans = w * 3;
      test_build_single(t, w, e, cbs, ans,
                        "All bridges available #3, iteration " +
...
// printVersion()
// Print grading test program version statement to cout - single line.
void printVersion() {
  const int VERSION = 5;
  std::cout << "Grading test program version " << VERSION
            << " - COMPILE WITH OPTIMIZATION ON" << std::endl;
}

// timesec
// Returns time in seconds since ... something
double timesec() {
...
int main() {
  Tester t;
  double time1 = timesec(); // Start time (sec)
  printVersion();
  test_build(t);
  std::cout << std::endl;
  printVersion();
  if (t.allPassed()) {
    std::cout << "All tests successful SO FAR; running EXTRA tests"
              << std::endl;
    std::cout << std::endl;
    test_build_EXTRA(t);
    printVersion();
  } else {
    std::cout << "EXTRA TESTS NOT DONE" << std::endl;
  }
  ...
  if (t.allPassed()) {
    ...
    std::cout << "Timing not done" << std::endl;
  }
  std::cout << std::endl;
